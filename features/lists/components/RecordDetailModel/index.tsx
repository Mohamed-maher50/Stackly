...
interface CardDetailModalViewProps {
  ...
  newComment: string;
  selectedPriority: ICard["priority"];
  ...
  onPriorityChange: (priority: ICard["priority"]) => void;
  onDueDateChange: (isoDate: Date) => void;
...
const RecordDetailModal = ({
  ...
  const priorityColors: Record<ICard["priority"], string> = {
    LOW: "text-blue-600",
  ...
  return (
    ...
    >
      {/* Header */}
      <DialogHeader className="bg-card border-b p-6 flex justify-between">
      ...
      <div className="p-6 space-y-6">
        ...
        <div className="grid grid-cols-2 gap-4">
          <div>
            ...
            {(["HIGH", "LOW", "MEDIUM", "URGENT"] as ICard["priority"][]).map(
              (p) => (
                <button
                  key={p}
                  onClick={(e) => {
          ...
          <div>
            ...
            <input
              type="date"
              value={card.dueDate?.toString()?.split("T")[0] || ""}
              onChange={(e) =>
                onDueDateChange(
            ...
            {card.comments.map((c, idx) => (
              <div key={idx} className="bg-muted p-3 rounded">
                <div className="text-xs text-muted-foreground">
                  {c.user} · {new Date(c.createdAt).toLocaleDateString()}
                </div>
                <p>{c.text}</p>
              </div>
            ))}
...
export default memo(RecordDetailModal);
